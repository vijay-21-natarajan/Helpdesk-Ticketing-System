...
def read_txt():
    ...
    with open('data.txt', mode='r') as file:
        ...
        for line in file:
            ...
            if len(fields) >= 12:  # Ensure there are at least 12 fields (including email)
                ticket = {
                    'id': fields[0],
                    'name': fields[1],
                    ...
                    'email': fields[11]  # Include email fieldS
                }
...
def write_to_txt():
    ...
    with open('data.txt', mode='w') as file:
        file.write("id,name,date,status,subject,department,location,room,mobile,time,priority,email\n")
        for ticket in tickets:
            file.write(f"{ticket['id']},{ticket['name']},{ticket['date']},{ticket['status']},{ticket['subject']},{ticket['department']},{ticket['location']},{ticket['room']},{ticket['mobile']},{ticket['time']},{ticket['priority']},{ticket['email']}\n")
...
def submit_ticket():
    ...
    ticket = {
        ...
        'time': time,
        'priority': False,
        'email': user_email  # Include the email in the ticket details
    }
...
def admin_edit_ticket(ticket_id):
    ...
    # Fetch the ticket before updating to use its details for the email
    ticket = next((t for t in ticket_tree.to_list() if t['id'] == ticket_id), None)
    ...
    if ticket:
        ...
        if new_status == 'Closed':
            send_closure_email(ticket['email'], ticket_id, ticket)
...
def send_closure_email(user_email, ticket_id, ticket_details):
    subject = f'Ticket Closed: {ticket_id}'
    body = f"""
    Dear {ticket_details['name']},

    Your ticket has been successfully closed. Here are the details:
    ...
    Thank you for raising the ticket. We hope your issue has been resolved.
    ...
    try:
        mail.send(msg)
        print(f"Closure email sent successfully to {user_email}")
    except Exception as e:
        print(f"Failed to send closure email to {user_email}. Error: {str(e)}")
